...
class Extractor(object):
    ...
    def __init__(self, fs, cal_val, tmpfs_path, data_path):
        ...
        self.mac_add = str(netmanager.getMac("eth0")).replace(":", "").lower()
    ...
    def write_spl_file(self, cur_time, z_spl_val, a_spl_val, c_spl_val):

        time_out = cur_time + 1

        try:
            if not os.path.isdir(self.laeq_store_dir):
                os.mkdir(self.laeq_store_dir)

            laeq_store_fname = "laeq.spl"
            laeq_store_full_path = os.path.join(self.laeq_store_dir, laeq_store_fname)
            with open(laeq_store_full_path, 'w') as outfile:
                # Write values to file - comma delimited
                outfile.write(np.char.mod('%.2f', z_spl_val))
                outfile.write(',')
                outfile.write(np.char.mod('%.2f', a_spl_val))
                outfile.write(',')
                outfile.write(np.char.mod('%.2f', c_spl_val))
                outfile.write(',')
                outfile.write(str(cur_time))
                outfile.write(',')
                outfile.write(str(time_out))

        except IOError as e:
            self.logger.error('Error writing to laeq file: ' + e.message)
        except OSError as e:
            self.logger.error('Error managing laeq file: ' + e.message)
        except Exception as e:
            self.logger.error('General error with laeq file: ' + e.message)

    def move_from_temp_file(self, st_time):
        ...
        try:
            self.logger.debug('Opening tar file: ' + tar_path)
            ...
            self.logger.debug('Copying slow file to SD: ' + data_path_name + '_slow.csv')
            shutil.copyfileobj(fsrc, fdst)
            fdst.flush()
            # tar.add(data_path_name + '_slow.csv')
            tar.add(data_path_name + '_slow.csv', arcname=self.mac_add + '_' + t + '_slow.csv')
            self.logger.debug('Slow file added to tar')
            fsrc = open(self.fast_path, "rb")
            fdst = open(data_path_name + '_fast.csv', "wb")
            fsrc.seek(0, 0)
            fdst.seek(0, 0)
            self.logger.debug('Copying fast file to SD: ' + data_path_name + '_fast.csv')
            shutil.copyfileobj(fsrc, fdst)
            fdst.flush()
            # tar.add(data_path_name + '_fast.csv')
            tar.add(data_path_name + '_fast.csv', arcname=self.mac_add + '_' + t + '_fast.csv')
            self.logger.debug('Fast file added to tar')

            tar.close()
            self.logger.debug('Tar file closed')
            os.remove(self.data_path + self.mac_add + '_' + t + '_slow.csv')
            os.remove(self.data_path + self.mac_add + '_' + t + '_fast.csv')
            self.logger.debug('Removed originals')

        except Exception, e:
            self.logger.error(
                'Error copying db data file from tmpfs to data folder and taring, data file may be lost: ' + str(e))

        try:
            self.logger.debug('Opening slow temp file for writing: ' + self.slow_path)
            self.temp_slow_data_file = open(self.slow_path, 'w')
            self.logger.debug('Opening fast temp file for writing: ' + self.fast_path)
            self.temp_fast_data_file = open(self.fast_path, 'w')

            self.logger.debug('Creating CSV writers')
            self.slow_csv_writer = csv.writer(self.temp_slow_data_file, delimiter=',')
            self.fast_csv_writer = csv.writer(self.temp_fast_data_file, delimiter=',')

            self.logger.debug('Writing column headers to temp files')
            self.write_row_to_file(self.col_heads, True, True)
            self.write_row_to_file(self.col_heads, False, True)
    ...
    def extract(self, st_time, sbuf):

        self.starttime = st_time
        ...
        # Add the Z, A & C values to the status file in tmp
        self.write_spl_file(self.starttime, slow_levels[0], slow_levels[1], slow_levels[2])

        # Add data to file as single csv row
        self.write_row_to_file(np.concatenate([np.array([self.starttime]), slow_levels, slow_octave_levels]), True,
                               False)
        ...
        if self.packet_idx == self.packet_delta:
            self.move_from_temp_file(self.starttime - (self.packet_delta - 1))
            self.packet_idx = 0
...
#
#     x = np.arange(48000)
#     test_signal = np.sin(2 * np.pi * 2000 * x / 48000)
#
